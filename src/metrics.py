...
def metrics(cbc, X_train, X_test, y_train, y_test, data, with_deals = 0):
    ...
    if with_deals:
        metrics_tr = calculate_metrics(y_downsampling(y_train, X_train, data), y_downsampling(y_pred_tr, X_train, data))
        metrics_ts = calculate_metrics(y_test, y_downsampling(y_pred_ts, X_test, data))
