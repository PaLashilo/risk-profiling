...
def adding_deal_sum_info(df: pd.DataFrame, data: dict) -> pd.DataFrame:
    ...
    non_zero_rows = df[df['mean_deal_sum'] != 0].drop(
        columns=["nickname", "id", "std_deal_sum", "class"])
    lin_reg = LinearRegression()
    lin_reg.fit(non_zero_rows.drop(
        columns=['mean_deal_sum']), non_zero_rows['mean_deal_sum'])
    zero_rows = df[df['mean_deal_sum'] == 0].drop(
        columns=["nickname", "id", "std_deal_sum", "class"])
    predicted_values = lin_reg.predict(
        zero_rows.drop(columns=['mean_deal_sum']))
    df.loc[df['mean_deal_sum'] == 0, 'mean_deal_sum'] = predicted_values

    non_zero_rows = df[df['std_deal_sum'] != 0].drop(
        columns=["nickname", "id", "class"])
    lin_reg = LinearRegression()
    lin_reg.fit(non_zero_rows.drop(
        columns=['std_deal_sum']), non_zero_rows['std_deal_sum'])
    zero_rows = df[df['std_deal_sum'] == 0].drop(
        columns=["nickname", "id", "class"])
    predicted_values = lin_reg.predict(
        zero_rows.drop(columns=['std_deal_sum']))
...
def adding_frequent_market_info(df: pd.DataFrame, data: dict) -> pd.DataFrame:
    ...
    for id in df.id:
        ...
        if table is not None:
            ...
            # market coefs
            stock_turnover = table.loc[1, 'part_turnover'] if table.loc[1,
                                                                        'part_turnover'] != '-' else 0
            curr_turnover = table.loc[3, 'part_turnover'] if table.loc[3,
                                                                       'part_turnover'] != '-' else 0
            fort_turnover = table.loc[2, 'part_turnover'] if table.loc[2,
                                                                       'part_turnover'] != '-' else 0
            ...
            # income
            df.income_percent_stats.loc[df.id ==
                                        id] = table.loc[0, "income_percent"]
    ...
    for id in df.id:
        ...
        if table is not None:
            ...
            if len(table) < 1:
                ...
                df.coef_free_funds.loc[df.id ==
                                       id] = std / table.start_sum.iloc[0]
